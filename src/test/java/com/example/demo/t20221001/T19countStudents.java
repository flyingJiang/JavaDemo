 ...
 * @author: jiangjianfei
 * @create: 2022-10-19 23:01
 **/
public class T19countStudents {
    ...
    public int countStudents(int[] students, int[] sandwiches) {
        ...
        while (!sandwichStack.isEmpty()) {
            if (sandwichStack.peek().equals(one) && !student.contains(one)) {
                return student.size();
            }

            if (sandwichStack.peek().equals(zero) && !student.contains(zero)) {
                return student.size();
            }
            if (sandwichStack.peek().equals(student.peek())) {
                sandwichStack.poll();
                student.poll();
                continue;
            }
            Integer toTail = student.poll();
            student.offer(toTail);
        }
        return 0;
    }
